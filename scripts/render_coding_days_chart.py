...
INPUT_CSV = REPO_ROOT / "img" / "coding-days-90d.csv"
META_JSON = REPO_ROOT / "img" / "coding-days-90d.json"
OUTPUT_SVG = REPO_ROOT / "img" / "coding-days-90d.svg"
...
def build_svg(owner: str, rows: list[tuple[str, int]], metadata: dict) -> str:
    ...
    lines.append(
        f'<text x="0" y="{height - 12}" fill="{muted}" font-family="Segoe UI, Helvetica, Arial, sans-serif" font-size="12">Updated: {escape(generated_label)}</text>'
    )
    lines.append("</svg>")
    return "\n".join(lines)
